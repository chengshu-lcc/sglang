...
from sglang_server.distribute.worker_info import g_worker_info, g_parallel_info, WorkerInfo
...
def members_from_json(gang_info_json: Dict[str, Any]) -> List[WorkerInfo]:
    ...
    for name, info in gang_info_json.items():
        ...
        node_rank = int(info['name'].replace("inference_part", "")) if 'name' in info else -1
...
def get_gang_info() -> GangInfo:
    if g_parallel_info.node_size > 1:
        # from config file
        ...
        members = [WorkerInfo(socket.gethostbyname(socket.gethostname()), -1, 0, -1, -1, -1, -1, -1, -1, -1, -1, 'local', None)]
    ...
    for part_rank, member in enumerate(members):
        ...
        if (new_member.ip == g_worker_info.ip and new_member.server_port == g_worker_info.server_port):
            self = new_member
        if part_rank == 0:
            master = new_member
