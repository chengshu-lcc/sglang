...
def start_api_server(extra_args=[]):
    ...
    # 在调用 prepare_server_args 之前，先转换远程路径
    processed_args = []
    i = 0
    while i < len(extra_args):
        ...
        if arg in ['--model-path', '--tokenizer-path', '--speculative-draft-model-path']:
            ...
            if i + 1 < len(extra_args):
                ...
                logger.info(f"Converting {arg} from remote path: {original_path}")
                local_path = fetch_remote_file_to_local(original_path)
                logger.info(f"Converted {arg} to local path: {local_path}")
    ...
    # 检查环境变量中的 TOKENIZER_PATH
    tokenizer_env = os.getenv("TOKENIZER_PATH")
    if tokenizer_env is not None:
        logger.info(f"Converting TOKENIZER_PATH from env: {tokenizer_env}")
        tokenizer_local = fetch_remote_file_to_local(tokenizer_env)
        logger.info(f"Converted TOKENIZER_PATH to local: {tokenizer_local}")
        os.environ["TOKENIZER_PATH"] = tokenizer_local

    logger.info(f"start_api_server processed_args: {processed_args}")
    server_args = prepare_server_args(processed_args)
    logger.info(f"start_api_server server_args prepared successfully")
